"""This module provides the CR To-DO CLI."""
# crtodo/cli.py
...
from crtodo import (
    ERRORS, __app_name__, __version__, config, database, crtodo
)

# ================== Initialize Library's functionalities
app = typer.Typer()
...
def remove(
    ...
    if force:
        ...
        if delete:
            ...
            typer.secho("Operation canceled", fg=typer.colors.YELLOW)
...
def get_todoer() -> crtodo.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please, run "crtodo init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return crtodo.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "crtodo init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
